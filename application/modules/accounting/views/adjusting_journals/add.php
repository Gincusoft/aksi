            ...
            <table class="table table-bordered table-condensed table-hover table-responsive" style="width: 100%">
              ...
              <tbody id="row-data">
                <tr>
                  ...
                  <td>
                    <select name="account_id[]" class="form-control" onchange="changeAccount(1)" id="account-id-1">
                  ...
                  <td><input type="text" name="debit[]" id="debit-1" class="form-control currency debit" onkeyup="doDebit()" value="0"></td>
                  <td><input type="text" name="credit[]" id="credit-1" class="form-control currency credit" onkeyup="doCredit()" value="0"></td>
                </tr>
              </tbody>
              <tfoot>
                <tr>
                    ...
                    $(document).ready(function() {
                      ...
                      $("#ff").validate({
                        ...
                        submitHandler: function(form) {
                          ...
                          var btn = $('#save_btn');
                          $.ajax({
                            url: '<?php echo base_url('accounting/adjusting_journals/add'); ?>',
                            type: 'post',
                            data: $('#ff').serialize(),
                            dataType: 'json',
                            beforeSend: function() {
                              if (checkBalance() == false) {
                                initCurrencyFormat();
                                showNotification('Debet & Kredit harus seimbang.', 'warning');
                                return false;
                              }
                              btn.button('loading');
                            },
...
//                      doCredit();
                    ...
                    function addRow() {
                      ...
                      //account
                      html += '<td><select name="account_id[]" class="form-control" onchange="changeAccount(' + rowId + ')" id="account-id-' + rowId + '"><option value="">- Pilih -</option>';
<?php
foreach ($account_list as $value) {
...
?>
                      html += '</select></td>';
                      html += '<td><input type="text" name="debit[]" id="debit-' + rowId + '" class="form-control currency debit" onkeyup="doDebit()"></td>';
                      html += '<td><input type="text" name="credit[]" id="credit-' + rowId + '" class="form-control currency credit" onkeyup="doCredit()"></td>';
                      html += '</tr>';
                    ...
                    function checkBalance() {
                      $('#debit-total').parent().removeClass('bg-red');
                      $('#credit-total').parent().removeClass('bg-red');
                      if ($('#debit-total').text() == $('#credit-total').text()) {
                        return true;
                      } else {
                        $('#debit-total').parent().addClass('bg-red');
                        $('#credit-total').parent().addClass('bg-red');
                        return false;
                      }
                    }
</script>