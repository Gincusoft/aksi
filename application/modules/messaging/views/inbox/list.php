...
<script>
        ...
        function doAdd() {
          showModal('<?php echo base_url('messaging/inbox/new_sms'); ?>', '#dest_number');
        }

        function doDelete(id) {
          showConfirmation('Lanjutkan menghapus pesan ini? ', 'confirm', function() {
            $.ajax({
              url: '<?php echo base_url('messaging/inbox/delete'); ?>',
              type: 'post',
              data: {inbox_id: id},
              dataType: 'json',
        ...
        $(document).ready(function() {
          ...
          $('#dataTable').DataTable({
            ...
            order: [[1, 'desc']],
            "ajax": {
              "url": "<?php echo base_url('messaging/inbox/get_list'); ?>",
              "data": function(d) {
            ...
            "columns": [
              ...
              {"data": "sms_date"},
              {"data": "sms_sender", orderable: false},
...
//                notifyMe(data.sms_sender, data.sms_text_short);
              }
            }
          });
          ...
          //Realtime updates
          var source = new EventSource("<?php echo base_url('messaging/inbox/check_inbox'); ?>");
