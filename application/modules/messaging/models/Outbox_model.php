 ...
 * @author tumbas
 */
class Outbox_model extends Tumbas_Model {
  ...
  function get_list($search, $order_column, $order_direction, $limit, $offset) {
    $where = "";
    if ($search != '') {
      $where .= " WHERE dest_no LIKE '%$search%' ";
    }

    $sql = " SELECT t1.*, pb.`name`, phonebook_group_name FROM (
						SELECT InsertIntoDB AS sms_date, DestinationNumber AS dest_no, TextDecoded AS sms_text, 'OnProcess' AS status FROM outbox ob JOIN (SELECT MAX(ID) AS max_id FROM outbox GROUP BY DestinationNumber) AS t1 ON t1.max_id = ob.ID
						UNION ALL
                        SELECT InsertIntoDB AS sms_date, DestinationNumber AS dest_no, TextDecoded AS sms_text, Status AS status FROM sentitems si JOIN (SELECT MAX(ID) AS max_id FROM sentitems GROUP BY DestinationNumber) AS t2 ON t2.max_id = si.ID
                ) AS t1
             LEFT JOIN t_mtr_phonebook pb ON pb.phone_number = t1.dest_no
...
//    die($this->db->last_query());
    $sql = "SELECT COUNT(DISTINCT dest_no) AS count FROM (
							 SELECT InsertIntoDB AS sms_date, DestinationNumber AS dest_no, TextDecoded AS sms_text FROM outbox
                             UNION ALL
                             SELECT InsertIntoDB AS sms_date, DestinationNumber AS dest_no, TextDecoded AS sms_text FROM sentitems
                ) AS t1 {$where}";
